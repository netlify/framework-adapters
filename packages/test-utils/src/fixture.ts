...
export class Fixture {
  ...
  async create() {
    if (!this.directory) {
      this.directory = await fs.mkdtemp(join(tmpdir(), 'netlify-framework-adapters-'))
  ...
  async destroy() {
    // There's not much use in cleaning up in CI, plus this fails in some cases on Windows images.
    if (process.env.CI) return

    await fs.rm(this.ensureDirectory(), { force: true, recursive: true })
  }
