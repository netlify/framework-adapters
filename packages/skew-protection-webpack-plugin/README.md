...
Pin a webpack SPA's asset requests to the Netlify deploy they were built from, so that an already-open tab keeps loading
the chunks it expects after a newer deploy ships, eliminating "Loading chunk failed" version-skew errors.
...
1. **Appends `?nfdpl=<token>` to dynamically-loaded chunk URLs** by wrapping the webpack runtime's chunk-URL function
   (`__webpack_require__.u`, and the mini-css-extract equivalent).
...
| `baseDir`   | compiler `context`                          | Where `.netlify/v1/` is written. Set to the Netlify base dir in a monorepo. |
